...
class MessageHandlingMixin:
    ...
    def _handle_start(self, message: types.Message) -> None:
        ...
        text = (
            f"{format_text('👋 ', mbold('Привет!'), separator='')}\n\n"
            f"{self._escape_markdown('Я — бот для общения с нейросетью GPT.')}\n\n"
            f"{self._escape_markdown('Просто напишите свой вопрос, задачу или идею — и получите ответ прямо здесь, в чате.')}\n\n"
            f"{self._escape_markdown('📌 Попробуйте начать с простого:')}\n"
            f"{self._escape_markdown('«Составь список дел на завтра»')}\n"
            f"{self._escape_markdown('или')}\n"
            f"{self._escape_markdown('«Объясни разницу между SEO и контекстной рекламой простыми словами»')}\n\n"
            f"{self._escape_markdown('✨ Чем точнее запрос, тем полезнее будет ответ.')}\n\n"
            f"{self._escape_markdown('Подробнее о том, как составить запрос можно узнать в разделе /help')}"
        )
        self._send_message(
    ...
    def _handle_help(self, message: types.Message) -> None:
        ...
        help_text = (
            f"{format_text('✍️ ', mbold('Как задавать запросы'), separator='')}\n\n"
            f"{mbold('Хороший ответ начинается с чёткого вопроса.')}\n"
            f"{self._escape_markdown('Если задача размыта, ответ получится общим. Сформулируйте: что именно нужно, в каком контексте и для какой цели. Чем яснее вопрос — тем точнее результат.')}\n\n"
            f"{mbold('Ставьте знак препинания в конце.')}\n"
            f"{self._escape_markdown('Иначе бот может продолжить вашу фразу вместо ответа.')}\n\n"
            f"{mbold('Уточняйте объём и формат.')}\n"
            f"{self._escape_markdown('Например: 200 слов, 500 символов, 3 предложения. Формат — список, инструкция, письмо, код.')}\n\n"
            f"{mbold('Добавляйте детали.')}\n"
            f"{self._escape_markdown('❌ «Расскажи про цветы»')}\n"
            f"{self._escape_markdown('✅ «Составь список из 5 самых популярных комнатных растений с кратким описанием ухода».')}\n\n"
            f"{mbold('Формулируйте запрос одним сообщением.')}\n"
            f"{self._escape_markdown('Бот не собирает несколько кусочков в единую задачу.')}\n\n"
            f"{self._escape_markdown('✨ Для сложных тем можно попросить уточняющие вопросы после описания задачи: «Задай мне уточняющие вопросы, чтобы я получил максимально точный ответ».')}"
        )
        self._send_message(
            chat_id=message.chat.id,
            text=help_text,
