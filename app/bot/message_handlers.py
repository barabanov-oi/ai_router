...
class MessageHandlingMixin:
    ...
    def _handle_start(self, message: types.Message) -> None:
        ...
        description = self._escape_html("Я — бот для общения с нейросетью GPT.")
        usage_hint = self._escape_html(
            "Просто напишите свой вопрос, задачу или идею — и получите ответ прямо здесь, в чате."
        )
        first_prompt = self._escape_html("📌 Попробуйте начать с простого:")
        todo_example = self._escape_html("«Составь список дел на завтра»")
        or_separator = self._escape_html("или")
        marketing_example = self._escape_html("«Объясни разницу между SEO и контекстной рекламой простыми словами»")
        accuracy_tip = self._escape_html("✨ Чем точнее запрос, тем полезнее будет ответ.")
        help_reference = self._escape_html("Подробнее о том, как составить запрос можно узнать в разделе /help")
        text = (
            "👋 <b>Привет!</b>\n\n"
            f"{description}\n\n"
            f"{usage_hint}\n\n"
    ...
    def _handle_help(self, message: types.Message) -> None:
        ...
        punctuation_tip = self._escape_html("Иначе бот может продолжить вашу фразу вместо ответа.")
        format_tip = self._escape_html(
            "Например: 200 слов, 500 символов, 3 предложения. Формат — список, инструкция, письмо, код."
        )
        bad_example = self._escape_html("❌ «Расскажи про цветы»")
        good_example = self._escape_html(
            "✅ «Составь список из 5 самых популярных комнатных растений с кратким описанием ухода»."
        )
        single_message_tip = self._escape_html("Бот не собирает несколько кусочков в единую задачу.")
        clarifying_tip = self._escape_html(
    ...
    def _handle_unknown_command(self, message: types.Message) -> None:
        ...
        self._send_message(
            chat_id=message.chat.id,
            text=self._escape_html("Команда не найдена."),
            escape=False,
        )
    ...
    def _handle_new_dialog(self, call: types.CallbackQuery) -> None:
        ...
        self._send_message(
            ...
            text=self._escape_html("🧹 Контекст очищен. Продолжайте беседу."),
    ...
    def _handle_dialog_history(self, call: types.CallbackQuery) -> None:
        ...
        self._send_message(
            chat_id=call.message.chat.id,
            text=self._escape_html("Выберите диалог из истории:"),
    ...
    def _handle_switch_dialog(self, call: types.CallbackQuery) -> None:
        ...
        if dialog_id is None:
            self._send_message(
                chat_id=call.message.chat.id,
                text=self._escape_html("Не удалось определить диалог"),
                escape=False,
            )
            return
        ...
        if not target_dialog:
            self._send_message(
                chat_id=call.message.chat.id,
                text=self._escape_html("Диалог не найден"),
                escape=False,
            )
            return
        ...
        if snippet:
            ...
            message_text = (
                f"{base_text}\n"
                f"{self._escape_html('📩 Последнее сообщение:')}\n"
                f"{quoted_snippet}"
            )
        else:
            message_text = (
                f"{base_text}\n"
                f"{self._escape_html('🚫 Последнее сообщение не найдено.')}"
            )
        self._send_message(
            chat_id=chat_id,
            text=message_text,
    ...
    def _handle_message(self, message: types.Message) -> None:
        ...
        try:
            ...
            if self._bot:
                self._send_message(
                    chat_id=message.chat.id,
                    text=self._escape_html(f"Произошла ошибка: {exc}"),
                    escape=False,
                )
        finally:
    ...
    def _escape_html(self, text: str | None) -> str:
        """Возвращает текст, безопасный для HTML-разметки Telegram."""
