...
class MessageHandlingMixin:
    ...
    def _handle_start(self, message: types.Message) -> None:
        ...
        text_fragments = [
            ("👋 <b>Привет!</b>", False),
            ("", False),
            ("Я — бот для общения с нейросетью GPT.", True),
            ("", False),
            (
            ...
            ("", False),
            ("📌 Попробуйте начать с простого:", True),
            ("«Составь список дел на завтра»", True),
            ("или", True),
            ("«Объясни разницу между SEO и контекстной рекламой простыми словами»", True),
            ("", False),
            ("✨ Чем точнее запрос, тем полезнее будет ответ.", True),
            ("", False),
            ("Подробнее о том, как составить запрос можно узнать в разделе /help", True),
        ]
        text = "\n".join(
            self._escape_html(content) if should_escape else content
            for content, should_escape in text_fragments
        )
        self._send_message(
    ...
    def _handle_help(self, message: types.Message) -> None:
        ...
        help_fragments = [
            ("✍️ <b>Как задавать запросы</b>", False),
            ("", False),
            ("<b>Хороший ответ начинается с чёткого вопроса.</b>", False),
            (
            ...
            ("", False),
            ("<b>Ставьте знак препинания в конце.</b>", False),
            ("Иначе бот может продолжить вашу фразу вместо ответа.", True),
            ("", False),
            ("<b>Уточняйте объём и формат.</b>", False),
            (
                "Например: 200 слов, 500 символов, 3 предложения. Формат — список, инструкция, письмо, код.",
                True,
            ),
            ("", False),
            ("<b>Добавляйте детали.</b>", False),
            ("❌ «Расскажи про цветы»", True),
            (
                "✅ «Составь список из 5 самых популярных комнатных растений с кратким описанием ухода».",
                True,
            ),
            ("", False),
            ("<b>Формулируйте запрос одним сообщением.</b>", False),
            ("Бот не собирает несколько кусочков в единую задачу.", True),
            ("", False),
            (
        ...
        help_text = "\n".join(
            self._escape_html(content) if should_escape else content
            for content, should_escape in help_fragments
        )
        self._send_message(
            chat_id=message.chat.id,
            text=help_text,
    ...
    # NOTE[agent]: Централизованное экранирование текста под HTML-разметку.
