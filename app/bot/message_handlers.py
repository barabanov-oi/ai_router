...
class MessageHandlingMixin:
    ...
    def _prepare_response_chunks(self, text: str, *, escape: bool = False) -> List[str]:
        ...
        processed_text = self._escape_html(text) if escape else (text or "")
        continuation = "..."
    ...
    def _handle_switch_dialog(self, call: types.CallbackQuery) -> None:
        ...
        title = self._escape_html(self._format_dialog_title(target_dialog))
        base_text = f"🔄 Переключаюсь на диалог <b>«{title}»</b>."
        reply_markup = self._build_inline_keyboard()
        ...
        if snippet:
            escaped_snippet = self._escape_html(snippet).replace("\n", "<br>")
            message_text = (
                f"{base_text}\n"
                "📩 Последнее сообщение:\n"
    ...
    def _handle_message(self, message: types.Message) -> None:
        ...
        try:
            ...
            if response_text and usage_summary:
                response_with_usage = f"{response_text}\n\n{usage_summary}"
            else:
                response_with_usage = response_text or usage_summary or ""
            reply_markup = self._build_inline_keyboard()
            if self._bot:
            ...
            if self._bot:
                safe_error = self._escape_html(str(exc))
                self._send_message(
                    chat_id=message.chat.id,
                    text=f"Произошла ошибка: {safe_error}",
                )
        finally:
    ...
    # NOTE[agent]: Централизованное экранирование пользовательского текста под HTML.
    ...
    def _send_message(
        ...
        **kwargs: Any,
        ...
        if not self._bot:
            return None
        safe_text = self._escape_html(text) if escape else text
